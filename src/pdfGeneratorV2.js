...
/**
 * Genera PDF de 2 páginas con el resultado del cálculo de OpenAI.
 * @param {Object} data  - Resultado de calcularLiquidacion()
...
function drawSignatures(doc, W) {
  const signW = W / 3 - 10;
  const labels = ['ADMINISTRACIÓN', 'REVISADO POR', 'RECIBIDO POR'];
  const y = doc.y + 15;
  labels.forEach((label, i) => {
    const x = 50 + i * (W / 3 + 5);
    doc.moveTo(x, y + 30).lineTo(x + signW, y + 30)
      .strokeColor('#AAAAAA').lineWidth(0.8).stroke();
    doc.fillColor(GRAY).fontSize(7.5).font('Helvetica')
      .text(label, x, y + 33, { width: signW, align: 'center' });
  });
}
...
function drawFooter(doc, W, page) {
  ...
  doc.fillColor(GRAY).fontSize(7).font('Helvetica-Oblique').text(
    `Página ${page} | Generado el ${ts} | LexBot – Sistema Legal Automatizado`,
